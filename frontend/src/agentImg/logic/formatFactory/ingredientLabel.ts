export type IngredientLabelLayoutType =
  | 'drug_facts'
  | 'supplement_facts'
  | 'standard'
  | 'nutrition_facts';
...
const renderDrugFacts = (sections: any[]) => {
  ...
  if (manuSec || ndcSec || lotSec || expSec || netSec) {
    ...
    if (manuSec) {
      ...
      const lines = wrapText(content, LINE_WIDTH, fontStr(infoFontSize));
      lines.forEach((line) => {
    ...
    if (netSec) {
      svg += `<text x="${START_X}" y="${y + infoLh}" font-family="Arial" font-weight="bold" font-size="${infoFontSize}">${escapeXml(netSec.content)}</text>`;
      y += infoLh + 2;
    }
  }
...
const renderSupplementFacts = (sections: any[]) => {
  ...
  if (Array.isArray(actives) && actives.length > 0) {
    actives.forEach((item: any) => {
      if (item.name) {
        const amount = item.amount === DEFAULT_MISSING_TEXT ? '0' : item.amount;
        const dv = item.dv === DEFAULT_MISSING_TEXT ? '0' : item.dv;
        if (dv === '*') hasAsterisk = true;
        if (dv === '†') hasDagger = true;

        const nameLines = wrapText(String(item.name), col1_W, fontStr(12));
        const rowH = Math.max(1, nameLines.length) * 14 + 2;

        nameLines.forEach((line, idx) => {
          svg += `<text x="${CONTENT_X}" y="${y + 12 + idx * 14}" font-family="Arial" font-size="12">${escapeXml(line)}</text>`;
        });

        svg += `<text x="${amountX}" text-anchor="middle" y="${y + 12}" font-family="Arial" font-size="12">${escapeXml(amount)}</text>`;
        svg += `<text x="${dvX}" text-anchor="end" y="${y + 12}" font-family="Arial" font-size="12">${escapeXml(dv)}</text>`;

        y += rowH;
        svg += `<line x1="${CONTENT_X}" y1="${y - 2}" x2="${CONTENT_END_X}" y2="${y - 2}" stroke="black" stroke-width="1" />`;
      }
    });
  } else {
    svg += `<text x="${CONTENT_X}" y="${y + 12}" font-family="Arial" font-size="12">${escapeXml(DEFAULT_MISSING_TEXT)}</text>`;
    y += 18;
  }
  ...
  if (hasAsterisk) {
    svg += `<text x="${CONTENT_X}" y="${y + 12}" font-family="Arial" font-size="10">* Daily Value (DV) not established.</text>`;
    y += 14;
  }
  if (hasDagger) {
    svg += `<text x="${CONTENT_X}" y="${y + 12}" font-family="Arial" font-size="10">† Daily Value (DV) not established.</text>`;
  ...
  const otherSecs = sections.filter(
    (s) =>
      !s.isHeader &&
      !s.isTable &&
      s.title.toUpperCase() !== 'SERVE HEADER' &&
      !s.title.toUpperCase().includes('SUPPLEMENT FACTS TABLE')
  );
  ...
  otherSecs.forEach((sec) => {
    ...
    y += 10;
    svg += `<text x="${CONTENT_X}" y="${y + 12}" font-family="Arial" font-weight="bold" font-size="12">${escapeXml(title)}:</text>`;

    const titleW = getCtx('bold 12px Arial')?.measureText(title + ': ').width || 0;

    let currentLine = '';
    const words = String(Array.isArray(content) ? content.join(', ') : content)
      .split(/\s+/)
      .filter((w: string) => w);
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const currentMaxWidth = contentParts.length === 0 ? CONTENT_W - titleW - 4 : CONTENT_W;
      if (measureTextWidth(attempt, FONT_BODY) <= currentMaxWidth) {
        currentLine = attempt;
      } else {
        if (currentLine) contentParts.push(currentLine);
        currentLine = word;
      }
    }
    if (currentLine) contentParts.push(currentLine);
    ...
    const firstLineWidth = measureTextWidth(firstLineContent, FONT_BODY);

    let firstLineX = CONTENT_X + titleW;
    let yOffset = 0;
    const SF_LINE_WIDTH = CONTENT_W - titleW - 4;
  ...
  sections.forEach((sec, idx) => {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const currentMaxWidth =
        contentParts.length === 0 ? STANDARD_LINE_WIDTH - titleW - 4 : STANDARD_LINE_WIDTH;
    ...
    let firstLineX = PADDING + titleW;
    let yOffset = 0;

    if (titleW + firstLineWidth < STANDARD_LINE_WIDTH) {
      firstLineX = PADDING + titleW;
      yOffset = LINE_H;
    } else {
      firstLineX = PADDING;
      yOffset = LINE_H * 2;
...
export const buildIngredientLabelSvg = (input: {
  productName: string;
  sections: any[];
  layoutType: IngredientLabelLayoutType;
}) => {
  const sections = Array.isArray(input.sections) ? input.sections : [];
  const layoutType = input.layoutType;

  let result: { svg: string; height: number };
  if (layoutType === 'drug_facts') {
    ...
    result = renderStandard(sections, input.productName);
  }
...
export const buildIngredientLabelSvgUrl = (svg: string) => {
