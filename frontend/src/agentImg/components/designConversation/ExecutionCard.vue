    ...
    <div class="execution-body">
      ...
      <div v-if="task?.status === 'success' && task.assets.length" class="deliverables">
        ...
        <a v-for="asset in task.assets" :key="asset.assetId" :href="taskAssetUrl(asset.assetId)" target="_blank" rel="noopener">
          <img v-if="asset.mimeType.startsWith('image/')" :src="taskAssetUrl(asset.assetId)" alt="Generated design" />
      ...
      <div v-if="run?.artifacts?.length" class="deliverables agent-deliverables">
        ...
        <a v-for="artifact in run.artifacts" :key="artifact.artifactId" :href="agentAssetUrl(artifact)" target="_blank" rel="noopener">
          <img v-if="artifact.mimeType.startsWith('image/')" :src="agentAssetUrl(artifact)" :alt="artifact.filename" />
          <span>
...
<style scoped>
.execution-card { max-width: 920px; margin: 2px auto 30px; border: 1px solid #cfd2c7; border-radius: 18px; overflow: hidden; background: #fbfcf7; box-shadow: 0 10px 30px rgba(16,17,15,.06); }
.execution-card > header { display: flex; align-items: center; gap: 12px; padding: 16px 18px; border-bottom: 1px solid #dfe1d8; background: #f3f4ed; }
.executor-icon { display: grid; place-items: center; width: 38px; height: 38px; border: 1px solid #171814; border-radius: 10px; background: #c7ff19; }
.executor-icon svg { width: 21px; height: 21px; fill: none; stroke: currentColor; stroke-width: 1.7; }
header p { margin: 0 0 2px; color: #6a6d64; font-size: 9px; font-weight: 820; letter-spacing: .12em; text-transform: uppercase; }
header h3 { margin: 0; font-size: 15px; }
.status-chip { display: inline-flex; align-items: center; gap: 7px; margin-left: auto; padding: 6px 9px; border: 1px solid #cfd2c7; border-radius: 999px; font-size: 10px; font-weight: 780; background: #fff; }
.status-chip i { width: 7px; height: 7px; border-radius: 50%; background: #7c8176; }
.status-running .status-chip i,.status-queued .status-chip i,.status-provisioning .status-chip i { background: #82a900; box-shadow: 0 0 0 3px rgba(199,255,25,.32); }
.status-failed .status-chip i { background: #bc3f35; }.status-waiting_budget .status-chip i,.status-waiting_authorization .status-chip i { background: #df9300; }.status-succeeded .status-chip i,.status-success .status-chip i { background: #168353; }
...
.plan-step:not(:last-child)::after { position: absolute; top: 25px; bottom: 3px; left: 13px; width: 1px; background: #d5d8cd; content: ''; }
.plan-step > span { position: relative; z-index: 1; display: grid; place-items: center; width: 28px; height: 28px; border: 1px solid #c7cabf; border-radius: 50%; color: #696c65; font-size: 10px; font-weight: 800; background: #fff; }
.plan-step p { margin: 5px 0 0; color: #63665e; font-size: 13px; }
.plan-step.completed > span { border-color: #151611; color: #151611; background: #c7ff19; }.plan-step.completed p { color: #1b1c19; }.plan-step.in_progress > span { border-color: #151611; box-shadow: 0 0 0 3px rgba(199,255,25,.35); }
.execution-meta { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; padding-top: 13px; border-top: 1px solid #e1e3db; }
.execution-meta span { display: flex; gap: 6px; padding: 6px 8px; border: 1px solid #dde0d6; border-radius: 8px; color: #555850; font-size: 10px; background: #fff; }.execution-meta b { color: #242622; }
.local-note,.upload-note,.budget-block { display: flex; align-items: center; gap: 12px; margin-top: 14px; padding: 12px; border: 1px solid #d4dfb7; border-radius: 10px; background: #f0f6dc; }
.local-note,.upload-note { flex-wrap: wrap; }.local-note b,.upload-note b { font-size: 12px; }.local-note span,.upload-note span { color: #5f6259; font-size: 11px; }
.upload-note { border-color: #c7d8f5; background: #eef4ff; }
.budget-block { justify-content: space-between; border-color: #e6c77f; background: #fff5dc; }.budget-block div { display: grid; gap: 3px; }.budget-block b { font-size: 12px; }.budget-block span { color: #6b5c37; font-size: 11px; }.budget-block a { color: #141510; font-size: 12px; font-weight: 800; }
.failure-block { display: grid; gap: 5px; margin-top: 14px; padding: 12px; border: 1px solid #d9aaa5; border-radius: 10px; background: #fff3f1; }.failure-block b { color: #8b3028; font-size: 12px; }.failure-block span { color: #66514e; font-size: 11px; line-height: 1.5; }
...
.deliverables > a { display: inline-flex; align-items: center; gap: 10px; min-width: 180px; max-width: 300px; margin: 0 8px 8px 0; padding: 8px; border: 1px solid #d7dad0; border-radius: 11px; color: #181915; text-decoration: none; background: #fff; }.deliverables img { width: 64px; height: 56px; border-radius: 7px; object-fit: cover; }.deliverables span { display: grid; gap: 3px; min-width: 0; font-size: 11px; }.deliverables b { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.deliverables small { color: #24704e; }
.approval-list { display: grid; gap: 10px; margin-top: 16px; }.approval-card { padding: 14px; border: 1px solid #d69a28; border-radius: 12px; background: #fffaeb; }.approval-kicker { color: #9c6500; font-size: 9px; font-weight: 850; letter-spacing: .1em; text-transform: uppercase; }.approval-card h4 { margin: 5px 0; }.approval-card p { margin: 0 0 6px; color: #575246; font-size: 12px; line-height: 1.5; }.approval-card small { color: #777165; }.approval-card .authorization-scope { margin-top: 10px; padding: 9px; border: 1px solid #e7d39e; border-radius: 8px; color: #5d5645; background: #fffdf6; }.approval-actions { display: flex; flex-wrap: wrap; gap: 7px; margin-top: 12px; }.approval-actions button,.approval-card a { min-height: 40px; padding: 8px 10px; border: 1px solid #31322d; border-radius: 8px; color: #fff; font-size: 11px; text-decoration: none; background: #1d1e1a; cursor: pointer; }.approval-actions .session { border-color: #7e9f18; color: #27300e; background: #fbfdef; }.approval-actions .deny { border-color: #a55c54; color: #8b3028; background: #fff; }
.execution-card > footer { padding: 0 18px 15px; }.progress-track { height: 3px; overflow: hidden; background: #e2e4dc; }.progress-track span { display: block; height: 100%; background: #82a900; transform-origin: left center; transition: transform 220ms ease; }.footer-actions { display: flex; align-items: center; gap: 8px; padding-top: 12px; }.footer-actions > span { margin-right: auto; color: #686b63; font-size: 10px; }.footer-actions button,.footer-actions a { padding: 8px 10px; border: 1px solid #c7cabf; border-radius: 8px; color: #262824; font-size: 11px; text-decoration: none; background: #fff; cursor: pointer; }.footer-actions .primary { border-color: #151611; font-weight: 780; background: #c7ff19; }.footer-actions .cancel { color: #9e3c33; }
@media (max-width: 640px) {
  .execution-card > header,.execution-body { padding: 14px; }.status-chip { max-width: 105px; }.execution-meta { display: grid; grid-template-columns: 1fr 1fr; }.budget-block { align-items: flex-start; }.approval-actions { display: grid; grid-template-columns: 1fr; }.approval-actions button { min-height: 44px; font-size: 12px; line-height: 1.35; }.footer-actions { flex-wrap: wrap; }.footer-actions > span { width: 100%; }.footer-actions button,.footer-actions a { flex: 1; text-align: center; }.deliverables > a { max-width: 100%; width: 100%; }
}
</style>
