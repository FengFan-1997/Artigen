...
备份/审计按 `PG_OPS_EXPECTED_MAJOR=16`（生产默认）或 `18`（托管 DEV）明确选择版本；
恢复由 manifest 固定同版本目标与客户端。工具的 Docker 合成数据演练与实际环境恢复记录分开，
具体命令和边界见 [生产 Runbook 的数据库备份章节](../PRODUCTION_RUNBOOK.zh-CN.md#4-数据库备份)。
