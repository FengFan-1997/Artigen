const installSystemRoutes = (app, deps) => {
  ...
  const trimJsonValue = (v, opts) => {
    ...
    const walk = (x, depth) => {
      ...
      if (depth >= maxDepth) return typeof x === 'object' ? '[truncated]' : null;
      if (Array.isArray(x)) return x.slice(0, maxItems).map((it) => walk(it, depth + 1));
      if (typeof x === 'object') {
        const out = {};
  ...
  const buildAgentImgDirectionPrompt = (input) => {
    const userInput = String(input?.userInput || '').trim().slice(0, 4000);
    const contextText = String(input?.contextText || '').trim().slice(0, 8000);
    const attempt = Number(input?.attempt || 0) || 0;
    const lang = detectUserInputLang(userInput);
    const zh = lang === 'zh';
    const memoryInputs = normalizeMemoryInputs(
      [...(Array.isArray(input?.memoryInputs) ? input.memoryInputs : []), ...splitUserInputSegments(userInput)],
      6
    );
    ...
    return [
      ...
      zh ? `用户输入（本轮原文）: ${userInput}` : `User input (verbatim, this turn): ${userInput}`,
  ...
  const buildAgentImgFinalPrompt = (input) => {
    ...
    const memoryText = formatUserInputsForPrompt(memoryInputs, lang);
    const opt = input?.option && typeof input.option === 'object' ? input.option : null;
    const optionText = opt
      ? safeJsonStringify({
        title: String(opt.title || '').trim().slice(0, 140),
        summary: String(opt.summary || '').trim().slice(0, 1200),
        styleTags: Array.isArray(opt.styleTags) ? opt.styleTags.slice(0, 24) : [],
        negativeTags: Array.isArray(opt.negativeTags) ? opt.negativeTags.slice(0, 24) : [],
        suggested: opt.suggested && typeof opt.suggested === 'object' ? opt.suggested : {}
      })
      : '';
    ...
    return [
      ...
      `Schema: ${schema}`,
      `User input language: ${lang}`,
      zh ? 'prompt 与 negativePrompt 必须使用中文，禁止出现英文。' : 'prompt and negativePrompt must be English. Do not output Chinese.',
      zh
      ...
      memoryText ? memoryText : '',
      zh ? `用户输入（本轮原文）: ${userInput}` : `User input (verbatim, this turn): ${userInput}`,
      optionText ? (zh ? `已选方向: ${optionText}` : `Chosen direction: ${optionText}`) : '',
      zh ? '只返回 JSON：prompt, negativePrompt, params' : 'Return JSON with: prompt, negativePrompt, params'
    ]
      .filter(Boolean)
      .join('\n\n');
  };
  ...
  const buildIngredientLabelPrompt = (input) => {
    const userText = String(input?.userText || '').trim().slice(0, 8000);
  ...
  app.get('/api/meta', rateLimit('meta', { max: 300, windowMs: 60 * 1000 }), (req, res) => {
    res.json({
      ok: true,
      nodeEnv: NODE_ENV,
      uptimeSec: Math.floor(process.uptime()),
  ...
  app.get('/api/health', rateLimit('health', { max: 120, windowMs: 60 * 1000 }), async (req, res) => {
    ...
    const result = {
      ...
      hf: {
        ...
        cache: {
          ...
          stats: typeof getHfCacheStats === 'function' ? getHfCacheStats() : null
        }
      },
      gemini: {
  ...
  const normalizeSelfBaseUrl = (raw) => String(raw || '').trim().replace(/\/+$/, '');
  ...
  const parsePrewarmUrls = (raw) => {
    ...
    for (const it of items) {
      try {
        const u = /^https?:\/\//i.test(it) ? new URL(it) : new URL(it.startsWith('/') ? `${base}${it}` : `${base}/${it}`);
  ...
  const runHfPrewarm = async ({ urls, mode, timeoutMs, concurrency }) => {
    if (typeof fetchWithTimeout !== 'function') return { ok: false, error: 'FETCH_NOT_AVAILABLE', items: [] };
    ...
    const timeout = Number.isFinite(t) && t > 0 ? Math.min(Math.max(t, 2000), 180000) : 45000;
    ...
    const worker = async () => {
      while (true) {
        ...
        try {
          ...
          const resp = await fetchWithTimeout(url, { method, headers, redirect: 'follow', compress: false }, timeout);
    ...
    return { ok: true, total: list.length, mode: m, items, elapsedMs: Math.max(0, Date.now() - startedAt) };
  };
  ...
  app.post(
    ...
    typeof rateLimit === 'function'
      ...
      : (req, res, next) => next(),
    async (req, res) => {
      if (typeof assertAdmin !== 'function') return res.status(501).json({ error: 'ADMIN_NOT_AVAILABLE' });
      ...
      const mode = normalizePrewarmMode(body.mode || process.env.HF_PREWARM_MODE || 'full');
      const timeoutMs = body.timeoutMs || process.env.HF_PREWARM_TIMEOUT_MS || 45000;
      const concurrency = body.concurrency || process.env.HF_PREWARM_CONCURRENCY || 2;
  ...
  const HF_PREWARM_ON_START = String(process.env.HF_PREWARM_ON_START || '').trim() === '1';
  if (HF_PREWARM_ON_START) {
    ...
    if (urls.length && typeof fetchWithTimeout === 'function') {
      setTimeout(() => {
        ...
        const mode = normalizePrewarmMode(process.env.HF_PREWARM_MODE || 'full');
  ...
  const GENERATE_RATE_WINDOW_MS = (() => {
    const v = Number.parseInt(String(process.env.GENERATE_RATE_WINDOW_MS || ''), 10);
    return Number.isFinite(v) && v > 0 ? v : 60 * 1000;
  })();

  const GENERATE_USER_MAX_CONCURRENCY = (() => {
    const v = Number.parseInt(String(process.env.GENERATE_USER_MAX_CONCURRENCY || ''), 10);
    return Number.isFinite(v) && v >= 0 ? Math.min(v, 20) : 2;
  })();
  const generateUserInFlight = new Map();
  const acquireGenerateUserSlot = (userId) => {
    if (GENERATE_USER_MAX_CONCURRENCY <= 0) return { ok: true, release: () => { } };
    ...
    if (n >= GENERATE_USER_MAX_CONCURRENCY) return { ok: false, release: () => { } };
  ...
  const GENERATE_IDEMPOTENCY_TTL_MS = (() => {
    const v = Number.parseInt(String(process.env.GENERATE_IDEMPOTENCY_TTL_MS || ''), 10);
    return Number.isFinite(v) && v > 0 ? Math.min(v, 24 * 60 * 60 * 1000) : 10 * 60 * 1000;
  })();
  ...
  app.post(
    '/api/generate',
    typeof rateLimit === 'function'
      ? rateLimit('generate', { max: GENERATE_RATE_MAX, windowMs: GENERATE_RATE_WINDOW_MS })
      : (req, res, next) => next(),
    async (req, res) => {
      const startedAt = Date.now();
      const requestId = String(req.body.requestId || res.locals.requestId || '');
      ...
      const requestSource = String(req.body.requestSource || '').trim().slice(0, 160);
      const pageContextRaw = req.body.pageContext;
      const pageContext =
        pageContextRaw && typeof pageContextRaw === 'object'
          ? trimJsonValue(pageContextRaw, { maxDepth: 6, maxItems: 80, maxString: 260 })
      ...
      if (
        ...
        try {
          if (purposeKey === 'agentimg_directions') prompt = buildAgentImgDirectionPrompt(input);
          if (purposeKey === 'agentimg_final') prompt = buildAgentImgFinalPrompt(input);
      ...
      const idempotencyKey = (() => {
        ...
        if (!rid) return '';
        const basis = userId || (typeof getClientIp === 'function' ? getClientIp(req) : '') || 'unknown';
      ...
      const finalizeIdempotency = (status, body) => {
        ...
        try {
          generateIdempotencyCache.set(idempotencyKey, { ts: Date.now(), status, body });
        } catch { }
        try {
      ...
      try {
        if (idempotencyKey) {
          ...
          if (cached && cached?.ts && Date.now() - cached.ts <= GENERATE_IDEMPOTENCY_TTL_MS) {
          ...
          if (inflight && typeof inflight.promise?.then === 'function') {
            ...
            const out = await Promise.race([
              inflight.promise,
              new Promise((resolve) => setTimeout(() => resolve({ status: 504, body: { error: 'UPSTREAM_TIMEOUT', requestId } }), waitMs))
            ]);
          ...
          generateIdempotencyInFlight.set(idempotencyKey, { ts: Date.now(), promise, resolve });
        }
        ...
        if (userId && !userId.startsWith('guest_') && typeof assertAuthUserMatches === 'function') {
          const auth = assertAuthUserMatches(req, res, userId);
          if (!auth) {
            const status = Number(res.statusCode || 401) || 401;
            const payload = { error: status === 403 ? 'FORBIDDEN' : 'UNAUTHORIZED', requestId };
        ...
        if (hold && !hold.ok) {
          ...
          const payload = { error: String(hold.error || 'CREDITS_ERROR'), requestId, ...(wallet ? { wallet } : {}) };
        ...
        const ingredientMeta = (() => {
          ...
          const ingredient = req.body?.ingredient;
          const input = agentImg || ingredient || {};
          const productType = String(input?.productType || '').trim().slice(0, 80);
          const parsed0 = extractFirstJsonObject(result?.text || '');
          const parsed =
            parsed0 && typeof parsed0 === 'object'
              ? trimJsonValue(parsed0, { maxDepth: 5, maxItems: 40, maxString: 800 })
              : null;
          const layoutType =
            parsed0 && typeof parsed0 === 'object'
              ? String(parsed0?.layoutType || '').trim().slice(0, 60) || null
              : null;
          const sectionTitles =
            parsed0 && typeof parsed0 === 'object' && Array.isArray(parsed0?.sections)
              ? parsed0.sections
                .map((it) => (it && typeof it === 'object' ? String(it.title || '').trim() : ''))
                .filter(Boolean)
                .slice(0, 40)
              : [];
          ...
          if (typeof appendUserImageHistory === 'function' && userId) {
            if (ingredientMeta && typeof ingredientMeta === 'object') {
              const entry = {
                ...
                purpose: purposeKey || (purpose ? normalizeReasonKey(purpose) : ''),
                productType: ingredientMeta.productType,
          ...
          if (typeof appendUserAuditHistory === 'function' && userId) {
            ...
            const inputTrimmed =
              input && typeof input === 'object'
                ? trimJsonValue(input, { maxDepth: 6, maxItems: 60, maxString: 1200 })
                : null;
            const entry = {
              id: requestId || `gen_${Date.now().toString(36)}`,
              ts: Date.now(),
              kind: 'generate',
              biz: purposeKey || (purpose ? normalizeReasonKey(purpose) : '') || 'generate',
              provider: String(result?.provider || activeTextProvider || 'text').trim(),
              model: String(result?.model || modelRaw || '').trim(),
              usedUrl: String(result?.usedUrl || '').trim() || undefined,
              cost: resolvedCost,
        ...
        const errorCode = (() => {
          ...
          if (failures.length) {
            ...
            if (statuses.some((s) => s === 500 || s === 502 || s === 503)) return 'SERVER_BUSY';
            ...
            if (failures.some((f) => (Number(f?.status || 0) || 0) === 0)) return 'NETWORK_ERROR';
          }
        ...
        const status = (() => {
          ...
          if (errorCode === 'UNAUTHORIZED' || errorCode === 'LOGIN_REQUIRED') return 401;
        ...
        try {
          const isIngredient =
            ...
            purpose.trim().toLowerCase() === 'ingredient_label' ||
            userText.toLowerCase().startsWith('ai_ingredient');
          if (isIngredient && typeof appendUserImageHistory === 'function' && userId) {
            const agentImg = req.body?.agentImg;
            const ingredient = req.body?.ingredient;
            const input = agentImg || ingredient || {};
            const productType = String(input?.productType || '').trim().slice(0, 80) || null;
            const entry = {
              id: requestId || `ingredient_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`,
              ts: Date.now(),
              type: 'ingredient_label',
        ...
        console.error('[API][Generate] Error:', { error: errorCode, requestId });
        const payload = {
          error: errorCode,
          requestId
        };
