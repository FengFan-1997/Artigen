      ...
      while (true) {
        ...
        try {
          ...
          const head = await client.query(
            `SELECT id FROM agent_provider_requests
              WHERE provider_key=$1 AND status='queued'
              ORDER BY priority,requested_at,id
              LIMIT 1`,
            [providerKey]
          );
          if (String(head.rows[0]?.id || '') === requestId && scheduler.rows[0]?.available === true) {
            await client.query(
              `UPDATE agent_provider_requests
...
const createModelCallService = ({ pool, retentionDays = 30 } = {}) => {
  ...
  const finish = async (call, {
    ...
    await pool.query(
      ...
      [
        ...
        Math.max(0, Number(queueWaitMs) || 0),
        Math.max(0, Date.now() - Number(call.startedAt || Date.now())),
  ...
  const summary = async ({ days = 7 } = {}) => {
    ...
    const [callsResult, qualityResult, runsResult, schedulerResult] = await Promise.all([
      pool.query(
    ...
    const [calls, quality] = await Promise.all([
      pool.query(
        `DELETE FROM agent_model_calls
          WHERE id IN (
            SELECT id FROM agent_model_calls
      ...
      pool.query(
        `DELETE FROM agent_quality_checks
          WHERE id IN (
            SELECT id FROM agent_quality_checks
             WHERE expires_at<=clock_timestamp()
             ORDER BY expires_at LIMIT $1
          )`,
        [bounded]
  ...
  return { cleanupExpired, start, finish, recordQualityCheck, summary };
};
