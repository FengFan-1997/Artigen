...
test('Live eval database readiness requires explicit PG18, dev_artigen and four free connections', async () => {
  const makePool = (row) => ({
    async query(input) {
      assert.match(String(input.text), /FROM pg_stat_activity/);
      assert.match(String(input.text), /WHERE backend_type = 'client backend'/);
      assert.doesNotMatch(String(input.text), /count\(\*\)::int FROM pg_stat_activity\) AS/);
