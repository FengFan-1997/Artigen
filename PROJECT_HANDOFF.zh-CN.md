...
- 设计对话和运行详情按服务端事件展示时间顺序的进展、可展开工具、用户补充、审批和结果；移除静态模拟进展。新增 owner 鉴权历史接口，独立 HTTP 游标分页恢复，与 SSE 按事件标识去重；详情切换任务时重建连接并隔离旧请求。
...
- 定向上下文回归 7/7，连同 Runtime 回归 199/199；两个相关页面的六配置浏览器回归 210/210，其中新增用例 24/24，另有任务切换及停止确认重置回归 6/6；`pnpm check:core` 通过（前端 224、后端 686 passed / 100 opt-in skipped、邮件 7，含类型、lint 与构建）。浏览器为 mock，真实 DEV 模型与部署尚未验收。
