...
## 2026-09-21 登录完成后的工作台刷新恢复（PR 候选，未部署）
...
- 同一用户 ID 登录/退出状态单测 2/2；Header、延迟登录、URL/最近会话、临时读取失败及新任务覆盖旧恢复请求在六个浏览器配置中 30/30 通过，既有设计对话回归 60/60；`pnpm check:core` 通过。全部 API 使用 mock，不包含真实登录或生产发布证明。


## 2026-09-21 设计对话文字输出约束修复（PR 候选，未部署）
