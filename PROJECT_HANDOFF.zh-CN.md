...
- 定向上下文回归 7/7，连同 Runtime 回归 199/199；两个相关页面的六配置浏览器回归 210/210，其中新增用例 24/24；`pnpm check:core` 通过（前端 224、后端 686 passed / 100 opt-in skipped、邮件 7，含类型、lint 与构建）。浏览器为 mock，真实 DEV 模型与部署尚未验收。
