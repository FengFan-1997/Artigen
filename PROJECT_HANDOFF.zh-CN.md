...
- 对齐后尝试新的小预算规划请求，但 Cloudflare 推理连续失败，设计任务未创建且没有冻结点数。调用记录只保存通用规划错误码，不能区分认证、限流、传输或上游故障；未再次创建 Agent Run，也未将先前中间产物或规划失败表述为成功验收。
...
## 2026-09-24 规划 Provider 错误分类（候选，未部署）

- 规划模型调用此前把 Cloudflare 的 HTTP/传输失败归并为 `DESIGN_PLANNER_FAILED`，同时丢弃失败状态，无法区分认证失败、限流、上游 5xx、响应无正文或网络故障。候选修复为调用日志生成不含响应正文、URL 和原始传输错误的状态分类；保留经过筛选的 HTTP 状态与耗时供重试策略使用。限流/超时/传输/5xx 继续按瞬时错误处理，凭据错误和不可用端点立即终止重试。
- 定向 Provider、运行时失败分类和规划错误处理回归 `86/86` 通过；`pnpm check:core` 通过（前端 `226`、后端 `693` 通过 / `100` 项 opt-in 跳过、邮件中继 `7`，含 lint、类型、构建与质量集 `50/50`）。隔离端口浏览器 E2E 为 `651` 通过、`3` 个明确禁用用例跳过；首次 `pnpm check` 因默认端口被另一工作树占用而停在 E2E 启动，随后改用 `ARTIGEN_E2E_PORT=51732` 单独重跑并通过。GitHub required CI、部署和 DEV 真实工作流复验尚未完成。此候选没有环境变量、模型、供应商、账号、数据库、存储、支付或生产配置变化。
