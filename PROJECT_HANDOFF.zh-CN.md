...
恢复加固的 DEV 运行时门槛已经满足。正式证据 PR 合入 `dev` 后可以建立 `dev → main` Release PR；生产发布必须让 Render、Vercel 与 Mac Worker 使用同一不可变最终 `main` SHA，并重新跑一次真实三子 Agent + Markdown/PDF smoke。生产成功前不能将修复标记为最终上线。Karing 只在全部联网发布与验收结束后从临时“全局”恢复为“规则”，不得修改节点、分流、DNS、系统代理或 Wi-Fi。
