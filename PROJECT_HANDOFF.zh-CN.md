...
- 候选部署后真实电脑任务确实执行了 Shell 并声称创建两份文件，但 Run 最终以 `AGENT_PLAN_INVALID` 失败；只读检查 DEV 持久记录确认没有完成记录或已持久化产物，临时沙箱文件未交付。根因是旧版 V1 在有效计划发布后重复拒绝无效计划更新，最终错误地终止整个任务；即使模型进入抑制分支，旧版仍把 `update_plan` 暴露给模型。该失败路径已在下一节修复候选中覆盖，不复用失败状态，也不把未落库的文件称为交付成功。
- 完整后端测试 `791` 项（`691` 通过、`100` 个 PostgreSQL/专用外部环境用例跳过）、后端 lint 已通过。浏览器真实交付及跨 Run 资产续做需待恢复候选通过 CI 并部署 DEV 后重新验收。

## 2026-09-24 Agent V1 无效计划恢复（修复候选，未部署）

- 当 Run 已发布有效计划后，模型连续两次提交无效计划更新，V1 现在保留服务器最后一份有效计划，继续完成既有任务；抑制更新后不再向模型提供 `update_plan`。这防止计划修订错误覆盖任务成果或把已执行工作直接判失败。
- 回归先提交有效计划，再提交两次无效更新，验证两次拒绝后保留旧计划、不再开放更新工具并继续完成回复。`pnpm --filter backend test`：`791` 项、`691` 通过、`0` 失败、`100` 个 PostgreSQL / 专用外部环境用例跳过；`pnpm --filter backend lint` 与 `git diff --check` 通过。
- 修复候选待 PR/完整 CI 和 DEV 部署后，重跑失败场景所对应的真实双文件电脑任务，并确认两份文件实际持久化、可下载、内容及脚本校验通过；在此之前不宣称真实闭环已完成。未修改数据库迁移、账号、平台配置、支付或生产。
