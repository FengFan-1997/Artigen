...
- 该实现已通过 PR [#153](https://github.com/FengFan-1997/Artigen/pull/153) 正常合入，并由硬锁修复 PR [#155](https://github.com/FengFan-1997/Artigen/pull/155) 与边界补丁 PR [#157](https://github.com/FengFan-1997/Artigen/pull/157) 补强；文档同步 PR [#158](https://github.com/FengFan-1997/Artigen/pull/158) 随后合入，当前 `dev` exact SHA 为 `61baf79df4042d7f2099ddbf334580ff0451b7f3`（运行时代码合入 SHA 为 `e3bd831610255733c5a60a379093f4c177af0476`）。Render/Vercel/Mac Worker 尚未以当前 exact SHA 对齐或部署；Runtime V2、公众 rollout 和 owner canary 继续关闭。
- 旧 Qwen 字样只保留在历史 Handoff、历史真实 Run 和确定性 fixture 兼容测试中，用于审计旧版本，不代表当前部署配置或新的真实调用方向。本轮未调用付费 Provider，也未执行 DEV/生产切换。
- PR #153、#155 和 #157 的 required checks 与 Release gate 均全部成功；#157 在 GitHub 更新到包含文档同步的最新 dev 基线后重新验证，覆盖 Core、Harness 五组、chaos、Chromium/Firefox/WebKit 与 Vercel。硬锁修复候选的定向 Agent/runtime/readiness/deployment 回归为 `167/167`，并额外覆盖 `NODE_ENV=test + APP_ENV=staging`，类型检查与 JS 语法检查通过。
...
- Cloudflare 实现已由 PR [#153](https://github.com/FengFan-1997/Artigen/pull/153) 合入 `dev` merge SHA `bc2f00530989dd39f3b0a85817ebe79210e31140`。新增 `cloudflare` Agent Provider：模型服务端硬锁 GPT-OSS 120B，Cloudflare API Origin 由严格校验的 account ID 拼接，Token 只从 Secret/Keychain 读取；Runtime V2 Prompt/Profile/Replay Oracle、durable model receipts、预算预留、工具白名单与结构化 JSON 纠错均按 Run 的固定 provider/model 工作，费用快照随不可变 Runtime Profile 持久化，终态清理和恢复不再误用 SiliconFlow 费率。
...
- 第二轮独立审核又关闭两类证据漂移：Live Eval 最终签名现在把实际文本/图片模型一起纳入 HMAC，owner canary 的运行配置必须与该模型锁完全一致；Worker 认领到与自身 provider/model 不同的旧 Run 时，会在读取私有任务、创建沙箱、调用模型或执行 Shell 前失败并释放未用冻结额度，禁止把旧 Qwen Run 静默改由 GPT-OSS 执行。DEV smoke 同时要求当前最新迁移 `026_agent_live_eval_capacity_counter`，不再把只到 025 的数据库误判为就绪。以上代码已合入 `dev`，但尚未部署。
...
## 2026-09-01 DEV 长期免费数据库隔离与最小权限收口（历史基线，统一候选已合入 dev 待部署）
...
- pg-boss 最小权限与连接容量门禁以及 Cloudflare 文本候选已经合入 GitHub `dev`，但 Render、Vercel、Mac Worker、浏览器、受限出口和桌面中继尚未以 `bc2f005…` 形成同一 SHA 的最终 DEV readiness。尚未对该 exact SHA 执行签名 gate、完整 24-slot 或图片匿名盲审；不得把早期单 Run 或本地门禁表述为整个 Agent 已完成上线。
...
- 真实生产游客流程只尝试一次 OTP：邮箱提交后显示发送失败，未收到验证码、未登录、未创建 Agent Run、未调用 Qwen/Kolors、未产生扣点。Render 恢复前禁止循环重发。当前机器的 DEV LaunchAgent 已加载但无 PID、最近退出码为 1；production LaunchAgent 仍有 PID `11287`，命令指向旧 worktree `Artigen-worker-production-25e09e2`，launchctl 记录最近状态 75。由于 Render/API 已停用，两者都不能作为 Worker、browser、restricted egress、desktop relay 或 durability pricing ready 的证据。
