...
## 2026-09-24 规划失败请求原位重试（开发候选，未部署）

- DEV 实际流程曾遇到规划 Provider 限流：系统明确说明“未创建 Agent 任务、未冻结点数”，但只让用户稍后重试，没有保留原请求的直接恢复动作。用户手动重发时容易重复消息，并可能丢失附件或已选产物上下文。
- 当前候选增加同一规划 job 的人工重试入口：只接受当前用户拥有的活动会话和已失败的原始用户消息；排队时复用加密保存的原文、附件和源产物，不重发、不复制消息。重试请求写入持久事件，任务状态在刷新后仍可恢复；并发中的 Agent Run、非失败 job、归档会话和未就绪的规划器均拒绝重试。每条原始请求一小时内最多人工重试 3 次。
- 失败消息只在其关联的原始请求仍失败、且它是该请求最新失败记录时显示“重新分析原需求”。按钮只重新执行无点数规划；只有后续展示报价并由用户明确启动，才可能创建付费任务或冻结点数。
- 本地 `pnpm check:core` 通过：前端 234 项、后端 698 通过 / 101 跳过、邮件 7 项，含 lint、类型检查、Agent 质量集、生产构建和预算校验；Chromium 定向 E2E 验证点击后复用原消息且没有发出新消息，通过。PostgreSQL 原位重试集成回归已添加，但本机未配置隔离测试数据库，按安全边界未连接 `.env` 数据库执行。真实 Provider 和 DEV 本轮未调用，PR 与部署待完成。
